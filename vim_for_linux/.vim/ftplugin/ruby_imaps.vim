...
" Ruby snippets
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
inoremap <buffer><localleader>ata <c-r>=IMAP_PutTextWithMovement("attr_accessor :<+ reader name +>")<cr>
...
inoremap <buffer><localleader>mo <c-r>=IMAP_PutTextWithMovement("module <+ module name +>\nend")<cr>
